...
public extension Graph {
    ...
    func dfs(fromIndex initalVertexIndex: Int,
        ...
        let neighbours = edgesIncidentFrom(index: initalVertexIndex, aligned: true)
        ...
        while !container.isEmpty {
            ...
            if shouldVisitNeighbours {
                visited[v] = true
                let neighbours = edgesIncidentFrom(index: v, aligned: true)
                for e in visitOrder(neighbours) {
                    if !visited[e.v] {
                        container.push(e)
                    }
    ...
    func dfs(fromIndex: Int, goalTest: (V) -> Bool) -> [E] {
        ...
        while !stack.isEmpty {
            ...
            for e in edgesIncidentFrom(index: v, aligned: true) {
                if !visited[e.v] {
                    stack.push(e.v)
                    pathDict[e.v] = e
                }
...
public extension WeightedGraph where W: Comparable & Numeric {
    ...
    func dijkstra(root: Int, startDistance: W) -> ([W?], [Int: WeightedEdge<W>]) {
        ...
        while let u = pq.pop()?.vertex { // explore the next closest vertex
            ...
            for we in edgesIncidentFrom(index: u, aligned: true)  { // look at every edge/vertex from the vertex in question
